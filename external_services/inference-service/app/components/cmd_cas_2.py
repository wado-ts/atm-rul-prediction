"""
RUL inference for CMD_CAS_2.

Owns loading and running CMD_CAS_2's trained model against the
...
def predict(sequence: dict[str, Any]) -> dict[str, Any]:
    """Run CMD_CAS_2's RUL model on its sequence."""
    model = get_model("CMD_CAS_2")
    bin_edges = get_bin_edges("CMD_CAS_2")
    ...
    device = get_device("CMD_CAS_2")

    X, mask = sequence_to_tensors(sequence, device)

    with get_model_lock("CMD_CAS_2"), torch.inference_mode():
