"""
RUL inference for component_5.

Owns loading and running component_5's trained model against the
sequence produced by app/components/component_5.py in the
...
def predict(sequence: dict[str, Any]) -> dict[str, Any]:
    ...
    Input:
        sequence: the component_5 sequence produced by
            build_sequence() in the sequence-building service's
            app/components/component_5.py.
    ...
    """
    model = get_model("component_5")
    raise NotImplementedError("Implement component_5 inference using `model`")
