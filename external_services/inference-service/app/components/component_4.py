"""
RUL inference for component_4.

Owns loading and running component_4's trained model against the
sequence produced by app/components/component_4.py in the
...
def predict(sequence: dict[str, Any]) -> dict[str, Any]:
    ...
    Input:
        sequence: the component_4 sequence produced by
            build_sequence() in the sequence-building service's
            app/components/component_4.py.
    ...
    """
    model = get_model("component_4")
    raise NotImplementedError("Implement component_4 inference using `model`")
